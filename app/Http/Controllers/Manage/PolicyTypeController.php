...
{
    public $layout = 'policyType';

    protected $module = 'PolicyType';
    ...
    public function add(Request $request){
        if ($request->isMethod('POST')){
            ...
            $this->validate($request,[
                'Data.type_name'=>'required|unique:zero_one_platform_policy.policy_type,type_name,NULL,id,is_del,0',
            ],[
    ...
    public function edit(Request $request, $id){
        ...
        if($info){
            if ($request->isMethod('POST')){
                $this->validate($request,[
                    'Data.type_name'=>'required|unique:zero_one_platform_policy.policy_type,type_name,'.$id.',id,is_del,0',
                ],[
                    'required'=>':attribute 为必填项',
                    'unique'=>':attribute 已存在',
                    'min'=>':attribute 长度不能小于2',
    ...
    protected function getResult($search, $row=0){
        ...
        if (isset($search['type_name'])){
            $obj = $obj->where('type_name','like','%'.$search['type_name'].'%');
        }
