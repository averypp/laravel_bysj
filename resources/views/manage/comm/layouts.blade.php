...
<head>
    ...
    @section('global_css')
        ...
        <link href="{{ asset('static/manage/css/myStyle.css') }}" rel="stylesheet">

        <link href="{{ asset('static/manage/css/bootstrap-select.css') }}" rel="stylesheet">
        <link href="{{ asset('static/manage/js/plugins/select2/css/select2.min.css') }}" rel="stylesheet">
...
@section('global_js')
    ...
    <!-- Flot -->
    <script src="{{ asset('static/manage/js/plugins/flot/jquery.flot.js') }}"></script>
    <script src="{{ asset('static/manage/js/plugins/flot/jquery.flot.tooltip.min.js') }}"></script>
    <script src="{{ asset('static/manage/js/plugins/flot/jquery.flot.resize.js') }}"></script>
    <script src="{{ asset('static/manage/js/plugins/flot/jquery.flot.pie.js') }}"></script>
    <script src="{{ asset('static/manage/js/jquery.cookie.js') }}"></script>

    <script src="{{ asset('static/manage/js/bootstrap-select.js') }}"></script>
    <script src="{{ asset('static/manage/js/plugins/select2/js/select2.min.js') }}"></script>
...
//                tokenSeparators: [',', ' ']
            ...
            $('.select2-tags').on("select2:select", function (e) {
                ...
                if(op != ''){
                    if (val!='' && val!=0 && val.match(/^[0-9]+$/) == null){
                        $.post('{{ url('/Manage/Tools/ajaxPutObject') }}',{op:op,name:val},function(_response) {
        ...
        function putObject(_obj) {
            ...
            if (name != 0){
                $.post('{{ url('/Manage/Tools/ajaxPutObject') }}',{op:op,name:name}, function (_response) {
                    if (_response.status == 1){
    ...
    <script type="text/javascript">
        ...
        function putTag(_obj,categoryId) {
            ...
            var name = $(_obj).parent().prev().val();
            if (name != 0){
                $.post('{{ url('/Manage/Tools/ajaxPutObject') }}',{op:op,name:name,categoryId:categoryId}, function (_response) {
                    if (_response.status == 1){
        ...
        function tagCategorySelect(_obj,index) {
            ...
            if (pid != 'none'){
                if (index < num) {
                    $.post('{{ url('/Manage/TagCategory/ajaxGetTagCategoryByPid') }}', {pid: pid}, function (_response) {
                        if (_response.status == 1) {
                ...
                if (index == num){
                    $.post('{{ url('/Manage/Tag/ajaxGetTagsByCategoryId') }}',{categoryId:pid}, function (_response) {
                        if (_response.status == 1){
    ...
    <script src="{{ asset('static/manage/js/pdata.js') }}"></script>
    <script type="text/javascript">
        $(function () {
            ...
            //绑定
            $("#search_province").val("{{ isset($search['province']) ? $search['province'] : '' }}");$("#search_province").change();
            $("#search_city").val("{{ isset($search['city']) ? $search['city'] : '' }}");$("#search_city").change();
            $("#search_area").val("{{ isset($search['area']) ? $search['area'] : '' }}");
        ...
        function detailedInformation(id,modelName) {
            if (id > 0){
                $.post('{{ url('/Manage/Tools/getDetail') }}',{id:id,modelName:modelName}, function (_response){
                    if (_response.status == 1){
