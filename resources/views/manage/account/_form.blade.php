...
<form method="post" class="form-horizontal" action="">
    ...
    <div class="form-group">
        ...
        <div class="col-sm-10">
            <input type="text" name="account[account]"
                   value="{{ old('account')['account'] ? old('account')['account'] : $account->account }}"
                   class="form-control">
        </div>
    </div>      
    ...
    <div class="form-group">
        ...
        <div class="col-sm-10">
            <input type="text" name="account[user_name]"
                   value="{{ old('account')['user_name'] ? old('account')['user_name'] : $account->user_name }}"
                   class="form-control">
        </div>
    </div>
    ...
    <div class="form-group">
        ...
        <div class="col-sm-10">
            <div class="radio i-checks">
                <label>
                    <input type="radio" value="1" name="account[sex]" {{ $account->sex == 1 ? 'checked' : '' }} /> <i></i>男</label>
            </div>
            <div class="radio i-checks">
                <label>
                    <input type="radio" value="0" name="account[sex]" {{ $account->sex == 2 ? 'checked' : '' }}> <i></i>女</label>
            </div>
    ...
    <div class="form-group">
        ...
        <div class="col-sm-10">
            <input type="text" name="account[position]"
                   value="{{ old('account')['position'] ? old('account')['position'] : $account->position }}"
                   class="form-control">
        </div>
    </div>
    ...
    <div class="form-group">
        ...
        <div class="col-sm-10">
            <input type="text" name="account[company]"
                   value="{{ old('account')['company'] ? old('account')['company'] : $account->company }}"
    ...
    <div class="form-group">
        ...
        <div class="col-sm-10">
            <div class="radio i-checks">
                <label>
                    <input type="radio" value="1" name="account[is_lock]" {{ $account->is_lock == 1 ? 'checked' : '' }} /> <i></i> 是</label>
            </div>
            <div class="radio i-checks">
                <label>
                    <input type="radio" value="0" name="account[is_lock]" {{ $account->is_lock == 0 ? 'checked' : '' }}> <i></i> 否</label>
            </div>
    ...
    <div class="form-group">
        ...
        <div class="col-sm-2 upPic" id="file-data-1">
            <img src="{{ $account->img  ? $account->img  : '/static/manage/img/icon-add.png' }}" onclick="file_select(1)" />
            <input type="hidden" name="account[img]"
                   value="{{ $account->img  ? $account->img : '' }}"
                   class="img">
    ...
    <div class="form-group">
        <div class="col-sm-4 col-sm-offset-2">
            ...
            <a class="btn btn-white" href="{{ url('/Manage/Account/index') }}">取消</a>
        </div>
    </div>
</form>
...
<script type="text/javascript">
    ...
    function _upload(_obj) {
        ...
        xhr.open("POST" ,"{{ route('manage.tools.uploadImg') }}" , true);
        xhr.send(fd);
